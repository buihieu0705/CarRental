            ...
            <c:if test="${sessionScope.user != null}">
                        ...
                        <div class="post type-post has-post-thumbnail">
                            <div class="post_wrapper grid_layout" style="padding: 0;">
                                <div  style="float: left; width:350px;">
                                    <img src="image/${requestScope.o.car.image}"  style="width:350px !important; height: 282.67px !important;"/>
                                </div>
                                <div  style="float: right; text-align: left; width: 575px;">
                                    ...
                                    <c:if test="${sessionScope.user.type == 2}">
                                        <c:if test="${fn:containsIgnoreCase(requestScope.o.status, 'request')}">
                                            ...
                                            <c:if test="${requestScope.isValidOrder}">
                                                <button class="button" style="width: 250px;"><a href="author?action=acceptOrder&orderId=${requestScope.o.id}" style="color: #fff;">Accept</a></button>
                                                <button class="button" style="width: 250px;"><a href="author?action=rejectOrder&orderId=${requestScope.o.id}" style="color: #fff;">Reject</a></button>
                                            </c:if>
                                            <c:if test="${!requestScope.isValidOrder}">
                                        ...
                                        <c:if test="${fn:containsIgnoreCase(requestScope.o.status, 'accept')}">
                                            <button class="button" style="width: 250px;"><a href="author?action=deliverCar&orderId=${requestScope.o.id}" style="color: #fff;">Car has been delivered</a></button>
                                        </c:if>
                                        <c:if test="${fn:containsIgnoreCase(requestScope.o.status, 'completed')}">
                                            <button class="button" style="width: 250px;"><a href="author?action=confirmCompletedHidden&orderId=${requestScope.o.id}" style="color: #fff;">The car has been paid</a></button>
                                        </c:if>
                                    ...
                                    <c:if test="${sessionScope.user.type == 1}">
                                        <h3 style="color: red">${requestScope.o.status}</h3>
                                        <c:if test="${fn:containsIgnoreCase(requestScope.o.status, 'request')}">
                                            <button class="button" style="width: 250px; margin-bottom: 10px; "><a href="author?action=cancel&orderId=${requestScope.o.id}" style="color: #fff;">Cancel</a></button>
                                        </c:if>
                                        <c:if test="${fn:containsIgnoreCase(requestScope.o.status, 'reject')}">
                                            <button class="button" style="width: 250px; margin-bottom: 10px; "><a href="author?action=confirmRejectHidden&orderId=${requestScope.o.id}" style="color: #fff;">Confirm rejected</a></button>
                                        </c:if>
                                        <c:if test="${fn:containsIgnoreCase(requestScope.o.status, 'deliverCar')}">
                                            <button class="button" style="width: 250px; margin-bottom: 10px; "><a href="author?action=confirmReceivedCar&orderId=${requestScope.o.id}" style="color: #fff;">Confirm car has been received</a></button>
                                        </c:if>
                                        <c:if test="${fn:containsIgnoreCase(requestScope.o.status, 'inprogress')}">
                                            <button class="button" style="width: 250px; margin-bottom: 10px; "><a href="author?action=completedOrder&orderId=${requestScope.o.id}" style="color: #fff;">Give car back</a></button>
                                        </c:if>
                                    </c:if>
                                    <c:if test="${sessionScope.user.type == 3}">
                                        <h3 style="color: red">${requestScope.o.status}</h3>
                                        <button class="button" style="width: 250px; margin-bottom: 10px; "><a href="orderForAdmin?action=edit&orderId=${requestScope.o.id}&page=booked" style="color: #fff;">Edit</a></button>
                                        <button class="button" style="width: 250px; margin-bottom: 10px; "><a href="orderForAdmin?action=delete&orderId=${requestScope.o.id}&page=booked" style="color: #fff;">Delete</a></button>
                                    </c:if>
                ...
                <c:if test="${i==0}">
                    ...
                    <c:if test="${sessionScope.user.type == 1}">
                        ...
                        <%request.setAttribute("href", "search?brand=&type=&sort_by=asc");%>
                    </c:if>
                    <c:if test="${sessionScope.user.type == 2}">
